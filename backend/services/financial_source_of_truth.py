...
# 'draft' is user-created; 'auto_draft' is auto-generated by the invoice
# creation flow at finance_invoices.py:736 & :804 (confirmed in the legacy
# invoice-status audit performed under Phase 2).
INFORMATIONAL_INVOICE_STATUSES = {"draft", "auto_draft", "finance_review", "approved"}
...
class FinancialSourceOfTruth:
    ...
    async def _compute_session_snapshot(
        ...
        # ---- Excluded-invoice snapshots (integrity-only; NEVER contribute to totals) ----
        # Non-revenue-eligible invoices (draft/auto_draft/voided/cancelled/deleted/
        # converted/unknown-status) still return net_invoiced_value = 0 and
        # outstanding_amount = 0 in their canonical snapshot. We compute the
        # snapshot solely to surface any integrity warnings attached to them
        # (e.g. NON_ELIGIBLE_INVOICE_HAS_ACTIVITY, VOIDED_INVOICE_HAS_ACTIVE_PAYMENTS,
        # UNRECOGNIZED_INVOICE_STATUS, UNRECOGNIZED_CREDIT_NOTE_STATUS).
        # Financial values are DELIBERATELY NOT added to any session total.
        excluded_invoice_summaries = []
        for inv in excluded_invoices:
            ...
            excluded_invoice_summaries.append({
                ...
                # Canonical contribution — always 0 by definition:
                "net_invoiced_value": 0.0,
                "outstanding_amount": 0.0,
            })
            warnings.extend(snap["integrity_warnings"])
