...
async def delete_session(session_id: str, current_user: User = Depends(get_current_user)):
    ...
    PHASE 3A (Section U): if the session has ANY financial history (issued/
    partially_paid/paid/terminal invoices, ANY payment including reversed,
    ANY credit note, or ANY journal entry) the request is REJECTED with
    HTTP 409 and the caller is directed to the /archive endpoint. Only
    sessions with pure safe-draft data may be hard-deleted.
    """
    if current_user.role != "admin":
    ...
    # ---- Phase 3A: financial-history safety check --------------------------
    from services.financial_write_guard import FinancialWriteGuard, is_production_mode
    guard = FinancialWriteGuard(db)
    history = await guard.session_has_financial_history(session_id)
    if history["has_history"] and is_production_mode():
        raise HTTPException(
            status_code=409,
            detail={
                "code": "SESSION_HAS_FINANCIAL_HISTORY",
                "message": (
                    "Session has financial history and cannot be hard-deleted. "
                    "Use /sessions/{session_id}/archive to archive it instead."
                ),
                "history": history,
            },
        )
...
async def archive_session(session_id: str, request: dict, current_user: User = Depends(get_current_user)):
    """PHASE 3A (Section U): archive a session while preserving all
    financial records (invoices, payments, credit notes, journal entries).

    Body:
        {"reason": "<why archived, minimum 5 chars>"}

    Only Admin/SuperAdmin may archive. Financial records are UNTOUCHED.
    """
    if current_user.role not in ("admin", "super_admin"):
        raise HTTPException(status_code=403, detail="Only admins can archive sessions")

    reason = (request or {}).get("reason", "").strip()
    if len(reason) < 5:
