...
async def _build_payment_history_query(
    ...
    # Status: 'active' (default in UI) => exclude reversed AND voided;
    # 'reversed' => only reversed; 'voided' => only voided;
    # 'inactive' => reversed or voided; 'all' => everything
    if status == "reversed":
        query["status"] = "reversed"
    elif status == "voided":
        query["status"] = "voided"
    elif status == "inactive":
        query["status"] = {"$in": ["reversed", "voided"]}
    elif status == "active" or not status:
        query["status"] = {"$nin": ["reversed", "voided"]}
...
async def record_payment(payment_data: PaymentCreate, current_user: User = Depends(get_current_user)):
    ...
    # ---- Phase 3A: canonical write guard -----------------------------------
    from services.financial_write_guard import (
        FinancialWriteGuard, FinancialSafetyError, validate_money,
    )
    guard = FinancialWriteGuard(db)
    try:
        amount = validate_money(payment_data.amount, field="payment amount")
        invoice = await guard.validate_payment_recordable(
            payment_data.invoice_id, amount,
        )
    ...
    if payment_data.create_credit_note and (payment_data.deduction_percentage or payment_data.deduction_amount):
        try:
            ...
            if deduction_amount > 0:
                ...
                # ---- Phase 3A (Sections L, N, M) ----
                # 1. Create CN with source_payment_id link (payment ↔ CN traceability).
                # 2. IMMEDIATELY set status="issued" — this is the canonical
                #    settlement CN. Draft/approved CNs no longer reduce net.
                # 3. Only then post to the accounting journal.
                credit_note = {
                    "id": str(uuid.uuid4()),
                    "cn_number": cn_number,
                    "invoice_id": payment_data.invoice_id,
                    ...
                    # Phase 3A: settlement CN starts issued (matches accounting posting below).
                    "status": "issued",
...
async def delete_payment(
    ...
    """PHASE 3A (Section K): hard-delete disabled in production.

    In production, this endpoint no longer physically removes a payment.
    Instead it converts the request into a controlled REVERSAL that
    preserves the original payment, receipt number, proof, created_at,
    original recorder, and invoice relationship.

    In test/dev environments (isolated test DB or APP_ENV in {test,dev}),
    hard delete remains available for cleanup fixtures.
    """
    if current_user.role not in ["admin", "finance"]:
    ...
    # --- Production path: delegate to the ONE canonical reversal engine ---
    ...
    if is_production_mode():
        ...
        result = await svc.execute(
            payment_id, reason=request.reason, user=current_user,
            alias="finance_admin_delete",
        )
    ...
    # --- Test/dev path: hard delete for fixture cleanup ---
    await create_audit_trail_entry(
        action="Payment Deleted (test/dev only)",
        record_reference=record_ref,
        entity_type="payment",
        entity_id=payment_id,
        changed_by=current_user,
    ...
    return {"message": "Payment deleted successfully", "reversed": False}
...
async def create_credit_note(cn_data: dict, current_user: User = Depends(get_current_user)):
    ...
    PHASE 3A: A valid, non-terminal, non-proforma invoice is REQUIRED.
    """
    if current_user.role not in ["admin", "super_admin", "finance", "coordinator"]:
        raise HTTPException(status_code=403, detail="Access denied")
    ...
    guard = FinancialWriteGuard(db)
    try:
        invoice = await guard.validate_credit_note_creation(cn_data.get("invoice_id"))
        # Amount validation — must be > 0.
        validate_money(cn_data.get("amount"), field="credit note amount")
    except FinancialSafetyError as e:
        raise HTTPException(status_code=e.http_status, detail={"code": e.code, "message": e.message})
    ...
    credit_note = {
        ...
        "invoice_number": invoice.get("invoice_number"),
        "session_id": cn_data.get("session_id") or invoice.get("session_id"),
        "session_name": cn_data.get("session_name") or invoice.get("session_name") or invoice.get("programme_name"),
        "company_id": cn_data.get("company_id") or invoice.get("company_id"),
        "company_name": cn_data.get("company_name") or invoice.get("company_name"),
        "bill_to_name": cn_data.get("bill_to_name") or invoice.get("bill_to_name"),
        "bill_to_address": cn_data.get("bill_to_address") or invoice.get("bill_to_address"),
        ...
        "base_amount": float(invoice.get("total_amount", 0)),
    ...
    # PHASE 3A: manual CN starts as DRAFT — journal posts only on /issue.
    # No auto-post at create time (removes the previous half-processed edge case).

    return {"message": "Credit note created (draft)", "cn_number": cn_number, "id": credit_note["id"]}
...
async def issue_credit_note(cn_id: str, current_user: User = Depends(get_current_user)):
    ...
    Phase 3A Section 6: this operation is ATOMIC — success means the CN is
    ``issued`` AND the accounting journal was posted. If accounting fails,
    the CN status is restored to its exact prior state and any partial
    journal side-effect is voided.
    """
    if current_user.role not in ["admin", "super_admin", "finance"]:
    ...
    prior_status = credit_note.get("status")
    prior_snapshot = {
        "status": prior_status,
        "issued_by": credit_note.get("issued_by"),
        "issued_at": credit_note.get("issued_at"),
    }
    now = get_malaysia_time()
    update_dict = {
        "status": "issued",
    ...
    if accounting_error:
        ...
        if prior_snapshot.get("issued_by") is not None:
            restore_doc["issued_by"] = prior_snapshot["issued_by"]
        ...
        await log_finance_action(
            ...
            after_value={"error": accounting_error},
        )
        raise HTTPException(
            status_code=500,
            detail={
                "code": "CN_ACCOUNTING_POST_FAILED",
                "message": (
                    "Credit Note journal posting failed; CN status restored. "
...
async def create_session_credit_note(session_id: str, cn_data: dict, current_user: User = Depends(get_current_user)):
    ...
    PHASE 3A (Section E): uses the canonical session → invoice resolver.
    - Never uses arbitrary find_one({"session_id": ...}).
    - Requires explicit ``invoice_id`` in ``cn_data`` when the session has
      more than one eligible real invoice.
    - Validates invoice belongs to the session, is a real invoice (not
      Proforma), and is not in a terminal (cancelled/voided/deleted/converted)
      state.
    """
    if current_user.role not in ["admin", "super_admin", "finance", "coordinator"]:
        raise HTTPException(status_code=403, detail="Access denied")
    ...
    guard = FinancialWriteGuard(db)
    try:
        invoice = await guard.resolve_session_primary_invoice(
            session_id, explicit_invoice_id=cn_data.get("invoice_id"),
        )
        # Also assert CN creation is allowed against that invoice (rejects
        # terminal statuses etc.).
        invoice = await guard.validate_credit_note_creation(invoice.get("id"))
    ...
    percentage = float(cn_data.get("percentage", 4))
    base_amount = float(cn_data.get("base_amount", invoice.get("total_amount", 0)))
    cn_amount = float(cn_data.get("amount", 0)) or (base_amount * percentage / 100)
    try:
        validate_money(cn_amount, field="credit note amount")
    except FinancialSafetyError as e:
        raise HTTPException(status_code=e.http_status, detail={"code": e.code, "message": e.message})
...
async def void_credit_note(
    ...
    # ---- Phase 3A (Section X): void the linked issued-CN journal entries.
    # Idempotent — journals already voided are skipped by the {status: {$ne: "voided"}}
    # filter. This mirrors the payment-reversal pattern in superadmin_portal.py.
        ...
        for je in cn_journals:
            await db.journal_entries.update_one(
                ...
                {"$set": {
                    ...
                    "voided_at": get_malaysia_time().isoformat(),
                    "void_reason": f"Credit note voided: {reason}",
                    "updated_at": get_malaysia_time().isoformat(),
                }}
            )
