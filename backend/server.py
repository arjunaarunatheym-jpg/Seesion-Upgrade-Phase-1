...
@api_router.get("/marketing/quotations/pending-approval")
async def get_pending_quotations(current_user: User = Depends(get_current_user)):
    """Get all quotations pending approval (admin only)"""
    if current_user.role not in ["admin", "super_admin"]:
        raise HTTPException(status_code=403, detail="Admin access required")
    
    quotations = await db.quotations.find({"status": "pending_approval"}, {"_id": 0}).to_list(1000)
    
    # Enrich with client and marketer info
    for q in quotations:
        client = await db.marketing_clients.find_one({"id": q.get("client_id")}, {"_id": 0})
        q["client"] = client
        
        marketer = await db.users.find_one({"id": q.get("created_by")}, {"_id": 0, "full_name": 1, "email": 1})
        q["marketer_name"] = marketer.get("full_name") if marketer else "Unknown"
