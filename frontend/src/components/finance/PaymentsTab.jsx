      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label data-testid="payment-amount-label">Payment Amount (RM)</Label>
              <Input
              ...
              {canonicalOutstanding && (
                <p className="text-xs mt-1" data-testid="canonical-outstanding-hint">
                  <span className="text-slate-500">Invoice Face Value:</span>{' '}
                  <strong>RM {Number(canonicalOutstanding.document_face_value || 0).toLocaleString('en-MY', {minimumFractionDigits: 2})}</strong>
                  {' · '}
                  <span className="text-slate-500">Canonical Outstanding:</span>{' '}
                  <strong className={canonicalOutstanding.outstanding_amount > 0 ? 'text-green-700' : 'text-slate-400'}>
                    RM {Number(canonicalOutstanding.outstanding_amount || 0).toLocaleString('en-MY', {minimumFractionDigits: 2})}
                  </strong>
                  {canonicalOutstanding.outstanding_amount <= 0 && (
                    <span className="text-red-600 ml-2" data-testid="invoice-fully-settled-badge">· Fully settled</span>
                  )}
                </p>
              )}
            </div>
