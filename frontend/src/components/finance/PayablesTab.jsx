...
const PayablesTab = ({
  ...
  const [currentPeriodStatus, setCurrentPeriodStatus] = useState({ status: 'open' });
  ...
  const handleClosePeriod = async () => {
    if (!confirm(`Close period ${payablesYear}-${String(payablesMonth).padStart(2, '0')}? This will lock all payables for this period.`)) return;
    try {
      await axiosInstance.post(`/finance/payables/close-period`, {
        year: payablesYear,
        month: payablesMonth
      });
  ...
  const handleReopenPeriod = async () => {
    if (!confirm(`Reopen period ${payablesYear}-${String(payablesMonth).padStart(2, '0')}?`)) return;
    try {
      await axiosInstance.post(`/finance/payables/reopen-period`, {
        year: payablesYear,
        month: payablesMonth
      });
  ...
  const handleExportPayablesExcel = async () => {
    try {
      const response = await axiosInstance.get(`/finance/payables/export?year=${payablesYear}&month=${payablesMonth}`, {
        responseType: 'blob'
      });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `payables_${payablesYear}_${payablesMonth}.xlsx`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      toast.success("Payables exported");
    } catch (error) {
      toast.error("Failed to export payables");
    }
  };
